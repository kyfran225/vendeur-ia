...
/**
 * `MapboxAddressAutofill` is a class that wraps an address [`<input>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/input/text)
 * element with intelligent, location-aware autocomplete functionality.
 *
 * Instantiating it produces a DOM element that you place inside a `<form>` to wrap one or more
 * `<input>` elements. `MapboxAddressAutofill` is also available as the `<mapbox-address-autofill>`
 * HTML custom element for declarative usage.
 *
 * To use this class, you must have a [Mapbox access token](https://www.mapbox.com/help/create-api-access-token/).
 *
 * The autofill element must be a descendant of a [`<form>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/form),
 * and the form must have inputs with proper HTML `autocomplete` attributes. If your application
 * works with browser autofill, you may already have this functionality.
