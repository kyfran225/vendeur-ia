...
export function StoreSetupWizardModal({
  ...
  return (
    <Portal.Root>
      <div className="fixed inset-0 z-[200] flex items-center justify-center p-3 sm:p-6 bg-black/40 dark:bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
        <motion.div
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <AddressAutocomplete
                      value={city}
                      onChange={setCity}
                      placeholder="Ex: Abidjan, Bouaké, Yamoussoukro, Cocody..."
                      inputClassName="w-full h-12 sm:h-13 px-4 rounded-2xl bg-slate-50 dark:bg-black/40 border border-slate-200 dark:border-white/10 focus:border-vendeur-emerald text-sm font-bold outline-none transition-all"
                    />
                  </div>
                </div>
              ...
              >
                ...
                <div className="space-y-3.5">
                  {deliveryFees.map((item, idx) => (
                    <div
                      key={idx}
                      className="relative p-3.5 pt-4 rounded-2xl bg-slate-50 dark:bg-black/30 border border-slate-200 dark:border-white/10 flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 sm:gap-3"
                    >
                      {/* Delete button positioned as top-right exposant badge */}
                      <button
                        type="button"
                        onClick={() => handleRemoveDeliveryZone(idx)}
                        className="absolute -top-2.5 -right-2.5 h-7 w-7 rounded-full bg-red-500 hover:bg-red-600 text-white flex items-center justify-center shadow-md hover:scale-110 transition-all shrink-0 cursor-pointer z-10"
                      ...
                      <input
                        ...
                        className="flex-1 h-11 px-3 rounded-xl bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 text-xs font-bold outline-none focus:border-vendeur-emerald min-w-0"
                      />
                      ...
                      <div className="relative w-full sm:w-36 shrink-0">
                        <input
                          ...
                          className="w-full h-11 pl-3 pr-14 rounded-xl bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 text-xs font-bold font-mono outline-none focus:border-vendeur-emerald text-right"
                        />
                        <span className="absolute right-3 top-1/2 -translate-y-1/2 font-mono font-bold text-[10px] text-slate-400 dark:text-white/40 pointer-events-none">
                          FCFA
                        </span>
                      </div>
                    </div>
              ...
              >
                ...
                <div className="space-y-3.5 pt-2">
                  {paymentMethods.map((item, idx) => (
                    <div
                      key={idx}
                      className="relative p-3.5 pt-4 rounded-2xl bg-slate-50 dark:bg-black/30 border border-slate-200 dark:border-white/10 flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3"
                    >
                      {/* Delete button positioned as top-right exposant badge */}
                      <button
                        type="button"
                      ...
                      <div className="w-full sm:w-36 font-black text-xs text-slate-900 dark:text-white truncate shrink-0">
                        {item.provider}
                      </div>
