...
class InstagramService {
  async handleIncomingMessage(pageId: string, senderId: string, text: string) {
    ...
    if (!merchant) {
      console.warn(`[Instagram] No merchant found for PageID ${pageId}`);
      return;
    }
    ...
    let customer = await CommerceCustomerModel.findOne({ merchantId: merchant._id, platformId: senderId, platform: "instagram" });
    if (!customer) {
      customer = await CommerceCustomerModel.create({
        merchantId: merchant._id,
        platformId: senderId,
        platform: "instagram",
        phone: "IG_" + senderId // Fallback phone identifier
      });
    }
