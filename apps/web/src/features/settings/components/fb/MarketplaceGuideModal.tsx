    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-3 sm:p-4">
      ...
      <div className="relative w-full max-w-lg bg-vendeur-bg border border-white/10 rounded-2xl sm:rounded-3xl p-4 sm:p-6 space-y-3.5 sm:space-y-4 shadow-2xl animate-in zoom-in-95 duration-200 max-h-[92vh] overflow-y-auto">
        ...
        <div className="text-center space-y-1.5 pt-1">
          <div className="h-12 w-12 bg-blue-500/10 border border-blue-500/20 rounded-2xl flex items-center justify-center mx-auto">
          ...
          <p className="text-white/50 text-xs max-w-sm mx-auto leading-relaxed">
            Pour automatiser vos ventes sur Marketplace en toute sécurité, Meta requiert une <strong className="text-white/80">Page Facebook Business</strong>.
          </p>
        </div>

        <div className="space-y-2">
          <h3 className="text-[10px] font-black uppercase tracking-wider text-white/30 text-center">Pourquoi une Page Business ?</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {[
              ...
              <div key={i} className="flex items-start gap-2.5 p-2.5 bg-white/[0.03] rounded-xl border border-white/5">
                <div className="h-5 w-5 rounded-full bg-blue-500/20 flex items-center justify-center shrink-0 border border-blue-500/40 mt-0.5">
                  <Check size={12} className="text-blue-400" />
                </div>
                <span className="text-[11px] font-medium text-white/70 leading-snug">{text}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="pt-2 border-t border-white/5 space-y-2">
          ...
          <button
            ...
            className="w-full h-9 sm:h-10 bg-white/5 text-white/60 hover:text-white hover:bg-white/10 font-bold uppercase tracking-wider text-[10px] sm:text-[11px] rounded-xl flex items-center justify-center gap-1.5 transition-all"
          >
