    ...
    <div className="min-h-[100dvh] bg-slate-50 dark:bg-vendeur-coal text-slate-900 dark:text-slate-100 flex flex-col selection:bg-vendeur-emerald selection:text-black transition-colors duration-200">
      ...
      <main className="flex-1 max-w-3xl mx-auto px-4 sm:px-6 py-12 w-full">
        <div className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-white/10 rounded-2xl p-6 sm:p-10 shadow-xl dark:shadow-2xl backdrop-blur-xl text-slate-800 dark:text-slate-300">
          <div className="border-b border-slate-200 dark:border-white/10 pb-6 mb-8 text-center sm:text-left">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-rose-500/10 border border-rose-500/20 text-rose-600 dark:text-red-400 text-xs font-semibold uppercase tracking-wider mb-4">
          ...
          <div className="space-y-6 text-slate-700 dark:text-slate-300 text-sm sm:text-base leading-relaxed">
            ...
            <div className="bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl p-5">
              <h3 className="font-bold text-slate-900 dark:text-white text-base mb-2 flex items-center gap-2">
                <span className="w-6 h-6 rounded-full bg-emerald-500 text-slate-950 text-xs font-extrabold flex items-center justify-center">1</span>
              ...
              {submitted ? (
                <div className="bg-emerald-500/10 border border-emerald-500/30 text-emerald-700 dark:text-vendeur-emerald p-4 rounded-lg text-xs font-medium flex items-center gap-3">
                  <ShieldCheck size={20} />
                ...
                <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3">
                  <input
                    ...
                    className="flex-1 bg-white dark:bg-slate-950 border border-slate-300 dark:border-white/20 rounded-lg px-4 py-2.5 text-sm text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:border-emerald-500 transition-colors shadow-inner"
                  />
                  <button
                    type="submit"
                    className="bg-rose-600 hover:bg-rose-500 text-white font-medium px-5 py-2.5 rounded-lg text-sm transition-colors flex items-center justify-center gap-2 shadow-md cursor-pointer"
                  >
                    <Trash2 size={16} />
            ...
            <div className="bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl p-5">
              <h3 className="font-bold text-slate-900 dark:text-white text-base mb-2 flex items-center gap-2">
                <span className="w-6 h-6 rounded-full bg-emerald-500 text-slate-950 text-xs font-extrabold flex items-center justify-center">2</span>
                Demande par e-mail
              </h3>
              <p className="text-xs text-slate-600 dark:text-slate-400">
                Vous pouvez également envoyer un message directement à notre responsable protection des données :
              </p>
              <div className="mt-3 text-xs bg-white dark:bg-slate-950 p-3 rounded-lg border border-slate-200 dark:border-white/10 flex items-center gap-3 text-slate-700 dark:text-slate-300">
                <Mail className="text-emerald-600 dark:text-vendeur-emerald shrink-0" size={18} />
                <div>
                  <strong>Email :</strong> <a href="mailto:vendeuria@gmail.com" className="text-emerald-600 dark:text-vendeur-emerald font-semibold hover:underline">vendeuria@gmail.com</a><br />
            ...
            {/* Warning info */}
            <div className="flex gap-3 bg-amber-500/10 border border-amber-500/20 text-amber-800 dark:text-amber-300 p-4 rounded-xl text-xs">
