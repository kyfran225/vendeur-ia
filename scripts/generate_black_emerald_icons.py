...
# 1. White rounded tile background (#ffffff) for max contrast in tabs
canvas = Image.new('RGBA', (h, w), (255, 255, 255, 255))
mask_bg = Image.new('L', (h, w), 0)
draw = ImageDraw.Draw(mask_bg)
draw.rounded_rectangle([(0, 0), (w, h)], radius=260, fill=255)

# 2. Draw Left Branch NOIR (#0f172a) & Right Branch VERT EMERAUDE (#10b981)
...
for y, x in zip(y_coords, x_coords):
    if alpha[y, x] > 100:
        if (x > 610 and y < 450) or (x > 650 and y >= 450):
            ...
            # Left Branch: Deep Black (#0f172a)
            logo_rgba[y, x] = [15, 23, 42, alpha[y, x]]

logo_img = Image.fromarray(logo_rgba, 'RGBA')

# Composite logo over white rounded tile
...
print('Saved Black & Emerald icons successfully!')
