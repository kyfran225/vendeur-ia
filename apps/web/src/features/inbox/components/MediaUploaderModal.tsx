    ...
    <div className="fixed inset-0 z-[999] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
      ...
      >
        ...
        <div className="p-6 flex flex-col items-center justify-center bg-[#0b141a]">
          {isImage && previewUrl ? (
            <div className="max-h-72 max-w-full rounded-xl overflow-hidden shadow-lg border border-[#222e35]">
              <img src={previewUrl} alt="Aperçu" className="max-h-72 w-auto object-contain" />
            </div>
          ) : (
            <div className="w-full p-6 rounded-xl bg-[#202c33] border border-[#2a3942] flex items-center gap-4">
        ...
        {/* Caption Input */}
        <div className="p-4 bg-[#202c33] border-t border-[#2a3942] space-y-3">
          <div className="relative">
            <input
              ...
              autoFocus
              className="w-full px-4 py-3 bg-[#111b21] border border-[#2a3942] rounded-xl text-white placeholder-[#8696a0] text-sm focus:outline-none focus:border-[#00a884]"
            />
          </div>
          ...
          <div className="flex items-center justify-end gap-3">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 rounded-xl border border-[#2a3942] text-xs font-semibold text-[#8696a0] hover:text-white"
            >
              Annuler
            </button>
            <button
              ...
              onClick={handleSend}
              className="px-5 py-2.5 rounded-xl bg-[#00a884] hover:bg-[#00a884]/90 active:scale-95 disabled:opacity-50 text-xs font-bold text-[#111b21] flex items-center gap-2 shadow-md transition-all"
            >
              {isUploading ? (
                <>
