...
class WhatsAppService {
    ...
    async handleIncomingMessage(userId, msg) {
        ...
        await CommerceMessageModel.create({ conversationId: conversation._id, sender: "customer", content: text });
        // Generate AI response
        const reply = await commerceService.processAiMessage(merchant._id, from, text, []);
        // Save and send AI message
        await CommerceMessageModel.create({ conversationId: conversation._id, sender: "ai", content: reply });
        const sock = this.activeSessions.get(userId);
        if (sock) {
            await sock.sendMessage(from, { text: reply });
        }
    }
    async sendMetaMessage(merchant, to, text) {
