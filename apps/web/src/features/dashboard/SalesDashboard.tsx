    ...
    <main className="max-w-6xl mx-auto p-4 md:p-10 space-y-8 pb-24 md:pb-8 animate-in fade-in duration-700">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-5 mb-2">
        <div className="space-y-1">
          ...
          <p className="text-slate-500 dark:text-white/50 text-xs sm:text-sm font-normal mt-1">Gérez votre croissance et suivez vos performances en direct.</p>
          {activeWhatsApp && (
            <div className="pt-1 w-full sm:w-auto">
              <button
                type="button"
                onClick={() => navigate("/settings?tab=connexions#whatsapp")}
                className="w-full sm:w-auto inline-flex items-center justify-between sm:justify-start gap-3 px-4 py-2.5 rounded-2xl bg-white dark:bg-white/5 hover:bg-slate-50 dark:hover:bg-white/10 border border-slate-200 dark:border-white/10 hover:border-vendeur-emerald/30 text-slate-900 dark:text-white transition-all group shadow-sm text-left cursor-pointer"
              ...
              >
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <div className="flex items-center gap-1.5 min-w-0">
                    <span className="text-[10px] text-slate-400 dark:text-white/40 font-bold uppercase tracking-wider hidden sm:inline">Ligne WhatsApp :</span>
                    <span className="text-xs sm:text-sm font-mono font-bold text-slate-900 dark:text-white group-hover:text-vendeur-emerald transition-colors truncate">
        ...
        <div className="hidden md:flex items-center gap-3">
          {canUpgradeToPro && (
            <button
              ...
              className="h-12 px-5 rounded-2xl bg-vendeur-emerald/10 border border-vendeur-emerald/30 text-vendeur-emerald text-xs font-black uppercase tracking-wider hover:bg-vendeur-emerald hover:text-slate-950 transition-all flex items-center justify-center gap-2 cursor-pointer active:scale-95 shadow-sm"
            >
              <Zap size={16} />
      ...
      */}
      {!isFounder && hasProducts && (
        <section className="relative overflow-hidden bg-white dark:bg-vendeur-coal/90 border border-slate-200 dark:border-white/10 p-4 sm:p-6 md:p-8 rounded-2xl sm:rounded-3xl md:rounded-[2.5rem] shadow-lg dark:shadow-2xl group space-y-4 sm:space-y-5">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 sm:gap-5">
            <div className="flex items-start sm:items-center gap-3.5 sm:gap-4 min-w-0">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 sm:gap-2.5 flex-wrap">
                  <h2 className="text-base sm:text-xl md:text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tight">
                    Votre Vitrine en Ligne
                  </h2>
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-vendeur-emerald/10 border border-vendeur-emerald/30 text-emerald-700 dark:text-vendeur-emerald text-[10px] font-black uppercase tracking-wider shrink-0">
                ...
                <p className="text-xs sm:text-sm text-slate-500 dark:text-white/50 font-medium mt-1 leading-snug">
            ...
            <div className="grid grid-cols-2 sm:flex sm:items-center gap-2 sm:gap-2.5 w-full lg:w-auto shrink-0">
              ...
              <Link
                ...
                target="_blank"
                className="flex items-center justify-center gap-1.5 sm:gap-2 min-h-[44px] sm:min-h-[48px] h-11 sm:h-12 px-3 sm:px-6 rounded-2xl bg-vendeur-emerald hover:bg-emerald-400 text-slate-950 font-black text-[11px] sm:text-xs uppercase tracking-wider transition-all shadow-lg shadow-vendeur-emerald/20 cursor-pointer active:scale-95 group hover:scale-[1.02] whitespace-nowrap"
          ...
          {/* Storefront Link Bar */}
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 p-1.5 sm:p-2 bg-slate-50 dark:bg-black/50 border border-slate-200 dark:border-white/10 rounded-2xl">
            <div className="flex-1 flex items-center gap-2 px-2.5 sm:px-3 py-1 sm:py-0 min-w-0 overflow-hidden">
              ...
              <span className="text-[11px] sm:text-xs font-mono font-bold text-slate-800 dark:text-white/90 truncate select-all">
            ...
            <button
              ...
              className={cn(
                ...
                hasCopiedShopUrl
                  ? "bg-vendeur-emerald text-slate-950 font-black"
                  : "bg-slate-200 hover:bg-slate-300 text-slate-900 dark:bg-white/10 dark:hover:bg-white/15 dark:text-white"
              )}
            >
              {hasCopiedShopUrl ? (
                <>
      ...
      <section className="bg-white dark:bg-vendeur-coal border border-slate-200 dark:border-white/5 rounded-3xl sm:rounded-[2.5rem] p-6 sm:p-8 shadow-lg dark:shadow-2xl">
        <h2 className="text-lg sm:text-xl font-black mb-6 uppercase tracking-tight text-slate-900 dark:text-white">Pipeline de Vente</h2>
      ...
      {dashboard?.merchant?.knowledge?.businessRules?.dynamicInsights?.length > 0 && (
        <section className="bg-white dark:bg-vendeur-coal border border-slate-200 dark:border-white/5 rounded-3xl sm:rounded-[2.5rem] p-6 sm:p-8 shadow-lg dark:shadow-2xl">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h2 className="text-lg sm:text-xl font-black uppercase tracking-tight text-slate-900 dark:text-white">Conseils Rentables de votre Vendeur IA</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {dashboard.merchant.knowledge.businessRules.dynamicInsights.slice(-4).reverse().map((insight: any, i: number) => (
              <div key={i} className="bg-slate-50 dark:bg-vendeur-bg border border-slate-200 dark:border-white/5 p-5 rounded-2xl flex items-start gap-4 hover:border-vendeur-emerald/30 transition-all">
                <div className={cn(
                  "h-9 w-9 rounded-xl flex items-center justify-center shrink-0",
                  insight.type === 'product' ? "bg-blue-500/10 text-blue-500 dark:text-blue-400" :
                  insight.type === 'customer' ? "bg-purple-500/10 text-purple-500 dark:text-purple-400" : "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400"
                )}>
                  {insight.type === 'product' ? <Package size={18} /> :
                   insight.type === 'customer' ? <AssistantIcon size={20} withBackground={false} /> : <TrendingUp size={18} />}
                </div>
                <div>
                  <p className="text-xs sm:text-sm font-medium leading-relaxed text-slate-800 dark:text-white/90">{insight.insight}</p>
                  <p className="text-[10px] text-slate-400 dark:text-white/30 uppercase font-black mt-2">Appris le {new Date(insight.createdAt).toLocaleDateString()}</p>
                </div>
              </div>
            ))}
          </div>
...
function MetricCard({ icon, label, value, suffix }: { icon: React.ReactNode; label: string; value: string; suffix?: string }) {
  return (
    <div className="bg-white dark:bg-vendeur-coal/50 backdrop-blur-sm border border-slate-200 dark:border-white/10 p-4 xs:p-5 md:p-6 rounded-2xl sm:rounded-[2rem] space-y-3 sm:space-y-4 shadow-md dark:shadow-xl hover:border-slate-300 dark:hover:border-white/20 transition-all group">
      <div className="h-10 w-10 sm:h-12 sm:w-12 bg-slate-100 dark:bg-white/5 rounded-xl sm:rounded-2xl flex items-center justify-center group-hover:scale-110 transition-transform">{icon}</div>
      <div>
        <p className="text-[10px] sm:text-xs font-black uppercase tracking-wider text-slate-500 dark:text-white/40 truncate">{label}</p>
        <div className="flex items-baseline gap-1 mt-0.5 sm:mt-1">
          <p className="text-xl sm:text-2xl md:text-3xl font-black text-slate-900 dark:text-white">{value}</p>
          {suffix && <span className="text-[10px] sm:text-xs font-black text-slate-400 dark:text-white/30 uppercase">{suffix}</span>}
        </div>
      </div>
    </div>
  );
...
function PipelineStep({ label, value, max, color }: { label: string; value: number; max: number; color: string }) {
  ...
  return (
    <div className="flex items-center gap-3 sm:gap-4">
      <div className="w-36 sm:w-48 text-xs sm:text-sm font-bold text-slate-700 dark:text-white/70 truncate">{label}</div>
      <div className="flex-1 h-3.5 bg-slate-100 dark:bg-white/5 rounded-full overflow-hidden">
        <div className={cn("h-full transition-all duration-1000", color)} style={{ width: `${percentage}%` }} />
      </div>
      <div className="w-12 text-right font-black text-xs sm:text-sm text-slate-900 dark:text-white">{value}</div>
    </div>
  );
}
