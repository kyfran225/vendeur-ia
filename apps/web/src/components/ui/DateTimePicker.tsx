...
export function DateTimePicker({ value, onChange, minDate = new Date() }: DateTimePickerProps) {
  ...
  return (
    <div className="w-full space-y-4 animate-in fade-in duration-300">
      <div className="grid grid-cols-1 md:grid-cols-12 gap-5 bg-black/40 border border-white/10 rounded-2xl p-3.5 sm:p-5">
        ...
        <div className="md:col-span-7 space-y-3">
          ...
          <div className="flex items-center justify-between px-1">
            <div className="flex items-center gap-2">
              <CalendarIcon size={15} className="text-sky-400" />
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                className="h-7 w-7 rounded-lg bg-white/5 hover:bg-white/10 text-white/60 hover:text-white flex items-center justify-center transition-colors cursor-pointer"
              >
                <ChevronLeft size={14} />
              </button>
              <button
          ...
          <div className="grid grid-cols-7 text-center">
            {DAYS_SHORT_FR.map((d) => (
              <span key={d} className="text-[9px] font-black uppercase text-white/30 tracking-widest py-0.5">
                {d}
              </span>
            ))}
          </div>
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    isSelected
                      ? "bg-sky-400 text-black font-black shadow-md shadow-sky-400/30 scale-105"
                      : isPast
                      ? "text-white/10 cursor-not-allowed"
                      : "text-white/80 hover:bg-white/10 hover:text-white",
                    isToday && !isSelected && "border border-sky-400/40 text-sky-400"
                  )}
                >
                  {day}
                  {isToday && !isSelected && (
        ...
        {/* Right: Time Selector */}
        <div className="md:col-span-5 border-t md:border-t-0 md:border-l border-white/5 pt-3 md:pt-0 md:pl-5 space-y-3 flex flex-col justify-between">
          <div className="space-y-3">
                  ...
                  return (
                    ...
                    >
                      <div>
                        ...
                        <p className="text-[8px] font-bold text-white/40 uppercase tracking-wider mt-0.5">{slot.tag}</p>
                      </div>
                      {isSlotSelected && <Check size={11} className="text-sky-400" />}
                    </button>
                  );
                })}
              </div>
            ...
            <div className="space-y-1 pt-1">
              ...
              <div className="flex items-center gap-2 bg-black/70 border border-white/10 px-3 py-1.5 rounded-xl">
                <select
                  value={selectedHour}
                  onChange={(e) => updateDateTime(selectedDate, parseInt(e.target.value, 10), selectedMinute)}
                  className="bg-transparent text-white font-mono font-bold text-xs sm:text-sm outline-none cursor-pointer flex-1"
                >
                  {Array.from({ length: 24 }).map((_, i) => (
                    <option key={i} value={i} className="bg-[#121614] text-white">
                ...
                >
                  {[0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55].map((m) => (
                    <option key={m} value={m} className="bg-[#121614] text-white">
