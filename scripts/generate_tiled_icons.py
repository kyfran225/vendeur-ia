...
# Create a dark tile background (#0f172a / Charcoal Black) with rounded corners
canvas = Image.new('RGBA', (h, w), (15, 23, 42, 255))

# Draw rounded rectangle mask on canvas
mask_bg = Image.new('L', (h, w), 0)
draw = ImageDraw.Draw(mask_bg)
draw.rounded_rectangle([(0, 0), (w, h)], radius=260, fill=255)

# Separate into left branch (white) and right branch (emerald)
...
# Composite logo over dark rounded tile
final_icon = Image.composite(canvas, Image.new('RGBA', (h, w), (0,0,0,0)), mask_bg)
final_icon.paste(logo_img, (0, 0), logo_img)

# Save crisp icons with dark tile container
sizes = {
...
print('Saved app icons with dark rounded tile successfully!')
